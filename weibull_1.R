...
# beta: şekil (shape), theta: ölçek (scale), gamma: konum (location)
...
#   E(Y^2) = E[(X+gamma)^2] = E(X^2) + 2*gamma*E(X) + gamma^2
#   E(Y^3) = E[(X+gamma)^3] = E(X^3) + 3*gamma*E(X^2) + 3*gamma^2*E(X) + gamma^3
#
# Önemli ilişkiler:
#   Var(Y) = Var(X) = theta^2 * [Gamma(1+2/beta) - Gamma(1+1/beta)^2]
#   Çarpıklık (Skewness): sadece beta'ya bağlı (theta ve gamma'dan bağımsız)
#
# STRATEJİ (Sende dediğin gibi):
...
# ==============================================================================
# BÖLÜM 2: MOM (Momentler Yöntemi) - SADE VERSİYON
# ==============================================================================
...
weibull_3p_mom <- function(y) {
  ...
  s2 <- var(y) * (n - 1) / n          # Varyans (n ile böl, n-1 değil)
  
  # Merkezi momentler
  m2 <- mean((y - y_bar)^2)           # 2. merkezi moment
  ...
  # Çarpıklık farkını minimize et
  objective_beta <- function(beta) {
    (weibull_skewness(beta) - skew_data)^2
  }
...
# burada t = ((y0 - γ) / θ)^β
#
# Benzer şekilde 2. ve 3. momentler için formüller türetilir.
# ==============================================================================
...
weibull_3p_mrl <- function(y, threshold_prob = 0.25) {
  ...
  mrl_objective <- function(par) {
    ...
    theo_2 <- theta^2 * exp_t * G2 - 2*z*theta*exp_t*G1 + z^2
    theo_3 <- theta^3 * exp_t * G3 - 3*z*theta^2*exp_t*G2 + 3*z^2*theta*exp_t*G1 - z^3
  ...
  if (!is.null(mom_est) && !is.na(mom_est$theta)) {
    start_values <- list(
      c(mom_est$theta, mom_est$beta, mom_est$gamma),
      c(mom_est$theta * 1.2, mom_est$beta * 0.8, mom_est$gamma * 0.9),
      c(mom_est$theta * 0.8, mom_est$beta * 1.2, mom_est$gamma * 1.1),
      c(sd(y), 1.5, y_min * 0.5),
      c(sd(y) * 1.5, 2.0, y_min * 0.3)
    )
  } else {
    start_values <- list(
      c(sd(y), 1.5, y_min * 0.5),
      c(sd(y) * 1.5, 2.0, y_min * 0.3),
      c(sd(y) * 0.7, 1.0, y_min * 0.7),
      c(mean(y), 2.0, 0),
      c(sd(y), 3.0, y_min * 0.8)
    )
  }
...
# ==============================================================================
# BÖLÜM 7: SİMÜLASYON ÇALIŞTIR
# ==============================================================================

cat("=====================================\n")
cat("SİMÜLASYON SONUÇLARI\n")
cat("=====================================\n\n")
...
# ==============================================================================
# BÖLÜM 8: MAKALE VERİSİ İLE TEST
# ==============================================================================
